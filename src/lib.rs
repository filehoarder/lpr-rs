...
impl LprConnection {
	...
	pub fn status(mut self) -> Result<String, Error> {
		...
		match bytes_written {
			...
			_ => Err(Error::new(ErrorKind::Interrupted, "not all bytes have been written")),
		}
	}
	...
	fn send_and_wait_for_ack(&mut self, data: &[u8], description: &str) {
		...
		self.stream.write(data).expect(&format!("writing {} to stream", description));

		let mut buf = [0; 1];
		self.stream.read(&mut buf).expect("reading acknowledge from stream");
		if self.verbose {
	...
	pub fn print_file(&mut self, path_to_file: &str) {
		...
		let file_size = file.read_to_end(&mut buf).expect("reading file into buffer");
		if self.verbose {
	...
	pub fn print_file_with_pjl_header(&mut self, path_to_file: &str, mut header_data: Vec<u8>) {
		...
		let _file_size = file.read_to_end(&mut file_buf).expect("reading file to buf");
